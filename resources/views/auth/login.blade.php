...
<head>
    ...
    <title>SIVSO | Sindicato de Oaxaca</title>
...
<body class="min-h-screen flex items-center justify-center bg-zinc-100 dark:bg-brand-black p-0 lg:p-8 transition-theme">
    ...
    <main class="w-full max-w-5xl lg:h-[600px] flex flex-col lg:flex-row overflow-hidden lg:rounded-2xl bg-white dark:bg-brand-dark shadow-[0_20px_50px_rgba(0,0,0,0.15)] dark:shadow-[0_20px_50px_rgba(0,0,0,0.4)] border-none lg:border border-zinc-200 dark:border-brand-border">
        ...
        <section class="w-full lg:w-5/12 relative overflow-hidden mobile-curve-container lg:h-full bg-white dark:bg-brand-dark">
            <img src="{{ asset('images/oficialLogin.jpg') }}" class="absolute inset-0 w-full h-full object-cover img-clear" alt="Fondo Institucional" onerror="this.style.display='none'">
        ...
        <section class="w-full lg:w-7/12 flex flex-col justify-center px-6 md:px-12 lg:px-16 py-10 lg:py-0 relative bg-white dark:bg-brand-dark z-10">
            ...
            <div class="w-full max-w-sm mx-auto">
                <header class="mb-10 text-center lg:text-left">
                    <div class="space-y-1">
                        <h2 class="text-2xl lg:text-3xl font-bold tracking-tight text-black dark:text-white">Inicio de Sesión</h2>
                        <p class="text-[10px] lg:text-[11px] text-zinc-500 dark:text-zinc-500 font-medium uppercase tracking-widest">Ingrese su RFC para acceder</p>
                    </div>
                </header>
                ...
                <form method="POST" action="{{ route('login') }}" class="space-y-6">
                    ...
                    <div class="space-y-1.5">
                        <label class="block text-[10px] uppercase tracking-[0.1em] text-zinc-500 font-bold ml-1">RFC</label>
                        <input type="text" name="rfc" value="{{ old('rfc') }}" placeholder="ABCD123456XYZ"
                            class="w-full px-4 py-3 bg-zinc-50 dark:bg-brand-black border border-zinc-200 dark:border-brand-border rounded-lg focus:ring-1 focus:ring-brand-gold outline-none transition-all placeholder:text-zinc-300 dark:placeholder:text-zinc-800 text-sm shadow-sm uppercase" required autofocus>
                    </div>

                    <div class="space-y-1.5">
                        <label class="block text-[10px] uppercase tracking-[0.1em] text-zinc-500 font-bold ml-1">Contraseña</label>
                    ...
                    <div class="flex items-center text-[11px] px-1">
                        <label class="flex items-center space-x-2 cursor-pointer group text-zinc-400">
                            <input type="checkbox" name="remember" class="rounded border-zinc-200 dark:border-zinc-800 bg-transparent text-zinc-900 focus:ring-0">
                            <span>Recordarme en este equipo</span>
                        </label>
                    </div>

                    <button type="submit" class="w-full bg-zinc-900 dark:bg-white text-white dark:text-black text-[11px] font-bold py-4 rounded-lg hover:opacity-90 transition-all uppercase tracking-[0.2em] shadow-lg">
                        Entrar al Sistema
                    </button>
                </form>

                <footer class="mt-12 pt-6 border-t border-zinc-50 dark:border-zinc-900">
                    <p class="text-[10px] text-zinc-400 dark:text-zinc-600 font-semibold uppercase tracking-[0.1em] text-center">
